...
```bash
make docker-build
docker inspect identity-service:$(git describe --tags --always --dirty) \
...
After unit and integration tests pass, CI builds `linux/amd64` and `linux/arm64` images and publishes `main`, `sha-*`, and semantic-version tags to `ghcr.io/lihongjie0209/identity-service`. Images include Git metadata, an SBOM, and build provenance. Pull requests build both architectures without publishing.
...
kubectl create secret generic identity-service --namespace platform \
...
kubectl wait --namespace platform --for=condition=complete job/identity-service-migrate --timeout=5m
...
Never reuse deleted field numbers or names: mark them `reserved`. Introduce breaking contracts under a new package such as `orders.v2`, and keep `v1` available until consumers have migrated. The module name in this template is `buf.build/lihongjie0209/identity-service`; publishing it requires a matching BSR account/module and a `BUF_TOKEN` secret. For an internal installation, replace the module name with the organization's BSR module.
