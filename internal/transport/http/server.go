...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/auth/mfa/verify", RateLimit(limiter, cfg.RateLimit.Login, "mfa-login", func(c *gin.Context) string { return c.ClientIP() }, logger), handler.VerifyMFAChallenge)
	...
	api.POST("/auth/password-reset/confirm", RateLimit(limiter, cfg.RateLimit.Login, "password-reset", func(c *gin.Context) string { return c.ClientIP() }, logger), handler.ConfirmPasswordReset)
	...
	api.POST("/service-accounts/get", handler.GetServiceAccount)
	api.POST("/service-accounts/list", handler.ListServiceAccounts)
	api.POST("/service-accounts/batch-get", handler.BatchGetServiceAccounts)
