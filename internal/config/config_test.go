...
func TestLoad_DefaultAudiencesIncludeRuleService(t *testing.T) {
	...
	if !slices.Contains(cfg.JWT.Audiences, "import-service") {
		t.Fatalf("JWT audiences = %v, want import-service", cfg.JWT.Audiences)
	}
}
...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/sessions/revoke, /api/v1/identities/update-status]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.identity.v1.IdentityService/RevokeTenantSessions]")
	cfg, err := Load(configPath)
	if err != nil {
		t.Fatalf("Load() error = %v", err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/sessions/revoke,/api/v1/identities/update-status" {
		t.Fatalf("HTTPPaths = %q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.identity.v1.IdentityService/RevokeTenantSessions" {
